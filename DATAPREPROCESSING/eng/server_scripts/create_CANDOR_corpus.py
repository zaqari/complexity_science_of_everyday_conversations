...
output_file_path = os.path.join('/home/zprosen/d/shapeoflang2', 'to_rosen')
...
for file_id_no, f in enumerate(files):
    ...
    main_data_file = [fi for fi in grab_all_files(temp_file, file_type='_cliffhanger.csv') if
                      (not fi.startswith('__MACOSX/')) and (not fi.startswith('._'))]
    main_survey_file = [fi for fi in grab_all_files(temp_file, file_type='survey.csv') if
                        (not fi.startswith('__MACOSX/')) and (not 'transcription/' in fi) and (not fi.startswith('._'))]

    # create CEDA ready corpus!
    for k, dfile in tqdm(list(enumerate(main_data_file))):
        try:
            CID = dfile.split('/')[-3]
            dft = pd.read_csv(dfile)
            dft['conversation_id'] = 'CANDOR-' + str(file_id_no) + '-' + CID
            dfs = pd.read_csv(main_survey_file[k])
            ...
            for i in dft.index:
                ...
                d_['conversation_id'] = dft['conversation_id'].loc[i]
            ...
            if data:
                ...
                data.to_parquet(
                    os.path.join(output_file_path, str(file_id_no) + '.parquet'),
                    engine='fastparquet',
                    compression='snappy'
                )

                # if os.path.exists(os.path.join(output_file_path, str(file_id_no) + '.tsv')):
                #
                #     data.to_csv(
                #         os.path.join(output_file_path, str(file_id_no) + '.tsv'),
                #         index=False,
                #         header=False,
                #         encoding='utf-8',
                #         mode='a',
